...
class TimeManagementApp(QMainWindow):
    ...
    def save_day_data(self):
        ...
        # Wczytaj istniejące dane lub utwórz nowy słownik
        try:
        ...
        # Przygotuj dane dla bieżącego dnia w nowej strukturze
        day_data = {}
        for row in range(24):
            for col in range(6):
    ...
    def load_day_data(self):
        ...
        try:
            ...
            # Nowy format - słownik {activity_name: [{row, col}, ...]}
    ...
    def save_data(self):
        ...
        msgBox.setText("Co chcesz zapisać?")
        btnAktywnosci = msgBox.addButton("Tylko aktywności", QMessageBox.ActionRole)
        btnDzienne = msgBox.addButton("Dane dzienne", QMessageBox.ActionRole)
        btnAnuluj = msgBox.addButton("Anuluj", QMessageBox.RejectRole)
        msgBox.exec_()

        if msgBox.clickedButton() == btnAktywnosci:
            # kod dla aktywności
            try:
        ...
        elif msgBox.clickedButton() == btnDzienne:
            # kod dla danych dziennych
            try:
                # Zbierz dane bieżącego dnia
                date_str = self.get_date_string()
                current_day_data = []
                for row in range(24):
                    for col in range(6):
                ...
                all_data[date_str] = current_day_data
                
                # Zapisz dane dzienne do wybranego pliku
                with open(file_name, "w") as file:
                    json.dump(all_data, file, indent=4)
            except Exception as e:
                QMessageBox.critical(self, "Błąd", f"Nie udało się zapisać danych dziennych: {str(e)}")
        # Jeśli reply == QMessageBox.Cancel, nie rób nic
    ...
    def load_data(self):
        ...
        try:
            ...
            with open(file_name, "r") as file:
                ...
                # Sprawdź format pliku - czy to lista aktywności czy dane dzienne
                if isinstance(loaded_data, list):
                    # To jest lista aktywności
                    self.activities = loaded_data
                    self.update_activity_list()
                    self.update_grid()  # Aktualizuj siatkę po wczytaniu nowych aktywności
                    self.load_day_data()  # Wczytaj dane dla bieżącego dnia
                elif isinstance(loaded_data, dict):
                    # To są dane dzienne - zapytaj użytkownika co chce zrobić
                    msgBox = QMessageBox()
                    msgBox.setWindowTitle("Wybór danych")
                    msgBox.setText("Wykryto dane dzienne. Co chcesz zrobić?")
                    btnAktywnosci = msgBox.addButton("Wczytaj tylko aktywności", QMessageBox.ActionRole)
                    btnDzienne = msgBox.addButton("Wczytaj dane dzienne", QMessageBox.ActionRole)
                    btnPolacz = msgBox.addButton("Połącz z istniejącymi danymi", QMessageBox.ActionRole)
                    btnAnuluj = msgBox.addButton("Anuluj", QMessageBox.RejectRole)
                    msgBox.exec_()

                    if msgBox.clickedButton() == btnAktywnosci:
                        # Ekstrakcja unikalnych aktywności z danych dziennych
                        unique_activities = set()
                        for date_data in loaded_data.values():
                            for item in date_data:
                                if "activity" in item:
                                    unique_activities.add(item["activity"])
                        
                        # Tutaj można dodać kod do konwersji nazw aktywności na pełne obiekty aktywności
                        # Na razie tylko wyświetlamy informację
                        QMessageBox.information(self, "Informacja", 
                                              f"Znaleziono {len(unique_activities)} unikalnych aktywności. Ta funkcja nie jest jeszcze zaimplementowana.")
                    
                    elif msgBox.clickedButton() == btnDzienne:
                        # Zastąp istniejące dane dzienne
                        try:
                            with open("time_data.json", "w") as file:
                        ...
                        # Wczytaj istniejące dane
                        try:
                            with open("time_data.json", "r") as existing_file:
                                existing_data = json.load(existing_file)
                                
                                # Połącz dane, nadpisując istniejące dni jeśli się powtarzają
                                for date_key, date_data in loaded_data.items():
                                    existing_data[date_key] = date_data
                            ...
                            # Jeśli nie ma istniejących danych, po prostu zapisz nowe
                            with open("time_data.json", "w") as file:
                                json.dump(loaded_data, file, indent=4)
                            self.load_day_data()  # Wczytaj dane dla bieżącego dnia
                        except Exception as e:
                            QMessageBox.critical(self, "Błąd", f"Nie udało się połączyć danych: {str(e)}")
                    
                    # Jeśli kliknięto Anuluj, nie rób nic
                else:
