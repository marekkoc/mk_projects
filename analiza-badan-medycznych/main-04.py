...
class KalendarzAktywnosci(QMainWindow):
    ...
    def rysuj_kalendarz(self):
        ...
        # Rysujemy heatmap - przesuwamy o 0.5 jednostki, aby komórki były wycentrowane na współrzędnych
        mesh = self.ax[0].pcolormesh(np.arange(54), np.arange(8), calendar_grid, cmap='viridis', vmin=0, vmax=9, edgecolors='gray', linewidth=1.0)
        
        # Ustawiamy etykiety osi
        self.ax[0].set_yticks(np.arange(0.5, 7.5))
        self.ax[0].set_yticklabels(['Pn', 'Wt', 'Śr', 'Cz', 'Pt', 'Sb', 'Nd'])
        
        # Ustawiamy etykiety miesięcy w prawidłowej kolejności
        month_positions = []
        month_labels = []
        month_names = ['Sty', 'Lut', 'Mar', 'Kwi', 'Maj', 'Cze', 'Lip', 'Sie', 'Wrz', 'Paź', 'Lis', 'Gru']
        
        for month in range(1, 13):
            # Obliczamy pozycję miesiąca w siatce
            first_day = pd.Timestamp(f'2023-{month:02d}-01')
            day_of_year = first_day.dayofyear - 1
            week_position = day_of_year // 7
            month_positions.append(week_position)
            month_labels.append(month_names[month-1])
        
        self.ax[0].set_xticks(month_positions)
        self.ax[0].set_xticklabels(month_labels)
        
        # Dodajemy tytuł osi X i Y dla lepszej czytelności
        self.ax[0].set_xlabel('Miesiące', fontsize=font_size_plot)
        self.ax[0].set_ylabel('Dni tygodnia', fontsize=font_size_plot)
    ...
    def on_click(self, event):
        if event.button is MouseButton.LEFT and event.inaxes == self.ax[0]:
            ...
            if 0 <= col < 53 and 0 <= row < 7:  # 53 tygodnie, 7 dni
                try:
                    # Znajdź pierwszy dzień roku
                    first_day = pd.Timestamp('2023-01-01')
                    # Znajdź pierwszy dzień pierwszego tygodnia ISO
                    first_week_day = first_day - pd.Timedelta(days=first_day.weekday())
                    # Oblicz datę na podstawie tygodnia i dnia tygodnia
                    date_idx = first_week_day + pd.Timedelta(weeks=col) + pd.Timedelta(days=row)
                    
                    # Sprawdzamy, czy data jest w naszym zbiorze danych i w 2023 roku
                    if date_idx in self.data.index and date_idx.year == 2023:
